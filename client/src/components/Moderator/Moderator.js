...
const ModeratorPage = () => {
  ...
  const fetchUserAccounts = async () => {
    try {
      ...
      return userData.filter(user => user.DoD_id !== token.DoD_id);
    } catch (error) {
  ...
  const UserAccountBanToggle = async (DoD_id, isBanned) => {
    try {
      ...
      await UserAccountStatus(DoD_id, updatedStatus);
  ...
  const UserAccountStatus = async (DoD_id, updatedStatus) => {
    try {
    ...
    <div  className="UserAccounts">
      ...
      {userAccounts.map((user) => (
        <div key={user.DoD_id}>
          ...
          <button className="ban-button" onClick={() => UserAccountBanToggle(user.DoD_id, user.isBanned)}>
      ...
      contestedPosts.map((post) => (
        <div key={post.SATCAT_id}>
          ...
          <button className="approve-button" onClick={() => contestedPostModeration(post.post_id, true)}>Approve</button>
          <button className="deny-button" onClick={() => contestedPostModeration(post.post_id, false)}>Deny</button>
